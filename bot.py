...
# BINANCE SPOT
# TELEGRAM ALERT
# FIXED BOOK-TICKER + REST SPREAD FALLBACK
# ============================================================
...
# Spread is now only a safety filter when real bid/ask exists.
# If WS book data is missing, REST fallback is used.
...
# REST spread fallback rate protection
REST_BOOK_TIMEOUT = 5
...
def load_all_histories():
    print(f"Loading 5M history for {len(symbols)} symbols...")
    ...
    with ThreadPoolExecutor(max_workers=15) as executor:
        ...
        for future in as_completed(futures):
            ...
            if completed % 50 == 0:
                print(f"History: {completed}/{len(symbols)}")
...
def find_resistance(symbol):
    ...
    for i in range(2, len(candles) - 2):
        ...
        left_highs = [candles[j]["high"] for j in range(i - 2, i)]
        right_highs = [candles[j]["high"] for j in range(i + 1, i + 3)]
    ...
    candidates.sort(key=lambda x: x["index"], reverse=True)
    ...
    for candidate in candidates:
        ...
        distance = ((current_price - resistance) / resistance) * 100.0

        # Do not use a resistance if price is already far above it.
        if distance > 5.0:
            continue

        tolerance = resistance * RESISTANCE_TOLERANCE_PERCENT / 100.0
        ...
        for i in range(len(candles)):
            ...
            if (
                ...
                and i - last_test_index < MIN_TEST_DISTANCE_CANDLES
            ):
                continue
...
def breakout_score(price, resistance):
    ...
    breakout = ((price - resistance) / resistance) * 100.0
...
def candle_quality(candle):
    ...
    close_position = ((close - low) / candle_range) * 100.0

    upper_wick = high - max(open_price, close)

    upper_wick_percent = (upper_wick / candle_range) * 100.0
...
def get_spread(symbol):
    ...
    # WebSocket book may not be ready yet.
    # Use REST fallback instead of rejecting the coin.
    now = time.time()
...
def analyze_symbol(symbol):
    ...
    buy_pressure = (taker_buy_quote / current_volume) * 100.0
    ...
    breakout_points, breakout_percent = breakout_score(
        price,
        resistance
    )
    ...
    breakout_volume_points, breakout_volume_ratio = breakout_volume_score(
        current_volume,
        average_volume
    )
    ...
    candle_points, close_position, upper_wick = candle_quality(candle)
    ...
    status = "🔥 STRONG BUY" if total_score >= STRONG_SIGNAL_SCORE else "🟢 BUY"

    entry = price

    stop = resistance * (
        1 - STOP_BELOW_RESISTANCE_PERCENT / 100.0
    )
...
def process_kline(symbol, k):
    ...
    with data_lock:
        ...
        if candle["closed"]:
            ...
            if (
                not history
                or history[-1]["open_time"] != candle["open_time"]
            ):
...
def book_ticker_worker(symbol_chunk):
    # IMPORTANT FIX:
    # Do not depend on the old global !bookTicker stream.
    # Subscribe directly to each symbol's bookTicker.
    streams = [
        f"{symbol}@bookTicker"
    ...
    url = (
        f"{BINANCE_WS}/stream?streams={stream_string}"
    )

    while running:
        try:
            print(
                f"BOOK WS connecting {len(symbol_chunk)} symbols..."
            )

            def on_open(ws):
                print(
                    f"BOOK WS CONNECTED {len(symbol_chunk)} symbols"
                )
            ...
            def on_close(ws, code, reason):
                print("BOOK WS closed:", code, reason)
...
def kline_websocket_worker(symbol_chunk):
    ...
    while running:
        try:
            print(
                f"KLINE WS connecting {len(symbol_chunk)} symbols..."
            )

            def on_open(ws):
                print(
                    f"KLINE WS CONNECTED {len(symbol_chunk)} symbols"
                )
            ...
            def on_close(ws, code, reason):
                print("Kline WS closed:", code, reason)
...
def status_worker():
    while running:
        ...
        if book_age >= 0:
            print("Last book update:", f"{book_age:.1f}s ago")
        else:
            print("Last book update: NEVER")
        ...
        print("Breakout volume passed:", current["breakout_volume"])
        print("Candle quality passed:", current["candle_quality"])
        ...
        print("Score >= 70:", current["score"])
        print("Telegram OK:", current["telegram_ok"])
        print("Telegram failed:", current["telegram_failed"])
        print("SIGNALS:", current["signals"])
        print("--------------------------------------")
        print("=========================================\n")
...
def volume_refresh_worker():
    ...
    while running:
        ...
        try:
            ...
            print("24H volume refreshed:", len(new_volume))
...
"""
    ...
    print("Final symbols:", len(symbols))
    ...
    # --------------------------------------------------------
    # WebSocket chunks
    # Both KLINE and BOOK use the same symbol chunks.
    # --------------------------------------------------------
